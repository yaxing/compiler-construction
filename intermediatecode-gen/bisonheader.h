...
void handleFuncProcCallParamIntermediateCode() {
    paramTypeList *cursor;
    quadrupleTable *table = getCurQuadrupleTable();
    cursor = paramTypeListHead;
    while(cursor != NULL) {
        if(cursor->typeInfo != NULL && cursor->typeInfo->code != NULL) {
            insertQuadruple(table, QUAD_OP_PARAM, buildStrArg(cursor->typeInfo->code), NULL, NULL);
        }
        cursor = cursor->next;
    }
}

void handleFuncProcCallIntermediateCode(struct TypeInfo **ruleHead) {
    handleFuncProcCallParamIntermediateCode();
    char *type = getIDName(predefinedIdTable, (*ruleHead)->typeEntry);
    arg *funcArg = buildStrArg((*ruleHead)->code);
    arg *tmp = NULL;
    char *op = QUAD_OP_CALL;
    if(strcmp(type, "function") == 0) {
        tmp = buildStrArg(createTmp());
        (*ruleHead)->code = tmp->argInfo.strInfo;
        op = QUAD_OP_FUNCALL;
    }
    insertQuadruple(getCurQuadrupleTable(), op, funcArg, NULL, tmp);
}
...
void handleArrayVar(struct IdResp **idResp, struct CURArrayType **arrayTypeInfo) {
    ...
    *arrayTypeInfo = (struct CURArrayType *)malloc(sizeof(struct CURArrayType));
    ...
    if(tmp->typedesc->attribute.arrayInfo.typeDefScopeId != curScope->scopeId) {
        //array defined in curscope, array type defined in parent
...
structuredStatement *buildstructuredStatementHeadType() {
    structuredStatement *tmp = (structuredStatement *)malloc(sizeof(structuredStatement));
    tmp->trueLabel = createLable();
    tmp->falseLabel = createLable();
    tmp->nextLabel = createLable();
    return tmp;
}
#endif
