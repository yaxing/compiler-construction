...
Program: PROGRAM ID SEMICOLON {
    insertQuadruple(getCurQuadrupleTable(), QUAD_OP_LABEL, buildStrArg($2->idStr), NULL, NULL);
} 

Program_Comp
;

Program_Comp: CompoundStatement DOT {
    if(MODE_DEBUG == 1) {
        printf("Program\n");
    }
}
| TypeDefinitions CompoundStatement DOT {
    if(MODE_DEBUG == 1) {
        printf("Program\n");
    }
}
| VariableDeclarations CompoundStatement DOT {
    if(MODE_DEBUG == 1) {
        printf("Program\n");
    }
}
| SubprogramDeclarations CompoundStatement DOT {
    if(MODE_DEBUG == 1) {
        printf("Program\n");
    }
}
| TypeDefinitions VariableDeclarations CompoundStatement DOT {
    if(MODE_DEBUG == 1) {
        printf("Program\n");
    }
}
| VariableDeclarations SubprogramDeclarations CompoundStatement DOT {
    if(MODE_DEBUG == 1) {
        printf("Program\n");
    }
}
| TypeDefinitions SubprogramDeclarations CompoundStatement DOT {
    if(MODE_DEBUG == 1) {
        printf("Program\n");
    }
}
| TypeDefinitions VariableDeclarations SubprogramDeclarations CompoundStatement DOT {
    if(MODE_DEBUG == 1) {
        printf("Program\n");
    }
}
...
Expression {
    insertQuadruple(getCurQuadrupleTable(), QUAD_OP_NOT, buildStrArg($3->code), NULL, buildStrArg(createTmp()));
    insertQuadruple(getCurQuadrupleTable(), QUAD_OP_IF,
        NULL, NULL, buildStrArg($1->nextLabel));
}
DO Statement {
...
ID ASSIGN Expression {
    insertQuadruple(getCurQuadrupleTable(), ":=", buildStrArg($5->code),
        NULL, buildStrArg($3->idStr));
}
TO Expression {
    insertQuadruple(getCurQuadrupleTable(), QUAD_OP_LABEL,
        buildStrArg($1->trueLabel), NULL, NULL);
    insertQuadruple(getCurQuadrupleTable(), ">",
        buildStrArg($3->idStr), buildStrArg($8->code), buildStrArg(createTmp()));
    insertQuadruple(getCurQuadrupleTable(), QUAD_OP_IF,
        NULL, NULL, buildStrArg($1->nextLabel));
}
DO Statement {
    ...
    insertQuadruple(getCurQuadrupleTable(), "+", buildStrArg($3->idStr),
    buildStrArg("1"), buildStrArg($3->idStr));
    insertQuadruple(getCurQuadrupleTable(), QUAD_OP_GOTO,
    buildStrArg($1->trueLabel), NULL, NULL);
    insertQuadruple(getCurQuadrupleTable(), QUAD_OP_LABEL,
    buildStrArg($1->nextLabel), NULL, NULL);
...
Expression {
    ...
    //handle inter-codegen
    insertQuadruple(getCurQuadrupleTable(), QUAD_OP_IF,
        NULL, NULL, buildStrArg($1->trueLabel));
    insertQuadruple(getCurQuadrupleTable(), QUAD_OP_GOTO,
        buildStrArg($1->falseLabel), NULL, NULL);
    insertQuadruple(getCurQuadrupleTable(), QUAD_OP_LABEL,
        buildStrArg($1->trueLabel), NULL, NULL);
...
ComponentSelection {
    ...
    if($3->typeEntry == -1) {
        ...
        constructTypeInfoFromIdResp(&$$, $1);
        $$->tag = ATTR_VAR;
        $$->code = popVarReferStack();
        if(strcmp($$->code, $1->idStr) != 0) {
            //meaning a tmp was generated for record selection (record)
            $$->code = getLastArg1(curQuadTable);
